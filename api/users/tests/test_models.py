...
class UserManagerTestCase(APITestCase):
    ...
    def test_create_user(self):
        fake = Faker()
        self.data['user_no'] = fake.random_number(digits=10, fix_len=True)
...
class UserTestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user = User.objects.create_user(user_no='user_no', password='password', name='name', email='email@sejong.ac.kr', user_type=admin_user_type)

    def test_pk_is_id(self):
        pk_name = User._meta.pk.name
        self.assertEqual(pk_name, 'id')

    def test_user_no_is_unique(self):
        is_unique = self.user._meta.get_field('user_no').unique
        self.assertTrue(is_unique)

    def test_user_no_max_length(self):
        max_length = self.user._meta.get_field('user_no').max_length
    ...
    def test_username_field(self):
        ...
        self.assertEqual(username_field, 'user_no')
    ...
    def test_str_representaion(self):
        self.assertEqual(str(self.user), 
                         f'{self.user.user_no} {self.user.name}({self.user.user_type})')
